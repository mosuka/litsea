...
### セグメンタにおける Packed スコアリング

`predict()` 自体は文字列で特徴量を検索し、そのセマンティクスは変わっていません。
ただし `Segmenter` はホットパスでこれを呼び出しません: モデルの（再）ロード後、
学習器の `(特徴量, 重み)` ペアを packed `u64` 整数キーへコンパイルし、各位置を
整数マップ検索でスコアリングします（[予測パイプライン](prediction-pipeline.md#packed-スコアリングテーブル)
を参照）。コンパイル済みテーブルはモデルを正確にミラーするため、両経路は同じ
スコアを生成します。
