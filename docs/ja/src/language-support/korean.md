...
- **注記**: 上記の `korean.model` とは異なり、このモデルは空白非保持の
  `word/POS` コーパスで**学習**されており、このページで説明している空白保持
  プロトコルは使用していません。held-out 品質は 2 通りで報告しています:
  空白非保持プロトコルの数値（単語 F1 83.24%、学習時と同じ方法で測定）と、
  自然なスペース付き入力で測定した実運用の数値（単語 F1 94.01%、issue #196）
  -- どちらも**同じモデル**の測定値であり、2 つの異なるモデルではありません。
  約 11 ポイントの差が、韓国語における空白非保持学習プロトコルの実際のコスト
  です。空白がないと境界の手がかりがより少ない言語でこの差がどれほど大きく
  なるかは[英語](english.md#english_posmodel)を参照してください
