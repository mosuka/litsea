...
- **コンパクトなモデル** -- レガシーな単語分割モデル（`RWCP.model`、`JEITA_Genpaku_ChaSen_IPAdic.model`）は約 16-22 KB。再学習された `japanese`/`chinese`/`korean`/`english.model` は約 86 KB〜2.0 MB で、[事前学習済みモデル](../pre-trained-models.md)に記載の品質向上とのトレードオフ。二段構成の品詞モデルは約 3.6-8 MB -- いずれもアプリケーションに直接埋め込める程度に小さく、重要な特徴量の重みのみを含む
