...
- New crate `litsea-wasm` (#206): the WebAssembly binding for browsers, Deno,
  ...
  release module is 178 KB, 82 KB gzipped. Exposes `Segmenter.fromBytes`,
  `segment`, `segmentBatch`, `segmentTokens`, `segmentWithPos`,
  `segmentWithPosBatch`, and `Token` with byte offsets; errors carry
  `err.code` as in the Node.js binding. Ships an optional
  `js/cache.js` helper that keeps fetched models in Cache Storage, since a
  model crosses the network once per visitor.
  Three things this host cannot do, each measured rather than assumed:
  **no `fromUri`** (reqwest's wasm backend has no `connect_timeout`, which
  `litsea::model_io` sets, so `--features remote_model` does not build for
  wasm32 -- the page fetches the model instead), **no training** (the
  extractor and trainers are path-based and wasm32 has no filesystem; #218
  tracks the in-memory APIs that would lift this), and consequently
  **no `CancelToken`**. Browser tests compare against fixtures
  generated by the `litsea` CLI, keeping the reference implementation as the
  source of truth.
...
- `litsea` no longer warns about an unused `BufReader` import when built for
  ...
  compiled out there. Noticed while adding `litsea-wasm` (#206).
