 ...
 * them and compares the result with `stubs/litsea.stubs.php`.
 *
...
final class StubGenerator
{
     ...
     *
     * ext-php-rs registers `#[php(prop)]` fields without a native type, so
     * reflection cannot tell `int` from `string`. The types are taken from the
     * Rust structs in `src/token.rs` and `src/metrics.rs` and written as
     * `@var` tags. Generation fails when a property is missing here or an
     * entry names a property that no longer exists, so this table cannot
    ...
    private const PROPERTY_TYPES = [
        ...
        'Litsea\MulticlassMetrics' => [
            'accuracy' => 'float',
            'macroPrecision' => 'float',
            'macroRecall' => 'float',
            'numInstances' => 'int',
        ],
        'Litsea\TwoStageMetrics' => [
            'stage1Accuracy' => 'float',
            'stage1NumInstances' => 'int',
            'stage2Accuracy' => 'float',
            'stage2NumInstances' => 'int',
        ],
    ];
    ...
    public function __construct(
        private string $extension = 'litsea',
        string $namespace = 'Litsea',
    ) {
    ...
    public function generate(): string
    {
        ...
        $out = "<?php\n\n";
        $out .= "/**\n";
        $out .= " * Stubs for the {$extension->getName()} PHP extension (Packagist: mosuka/litsea).\n";
        $out .= " *\n";
        $out .= " * Generated from the compiled extension by litsea-php/tools/generate-stubs.php;\n";
     ...
     * side effects for any litsea class.
     *
     * @param ReflectionClass<object> $class
     */
    ...
    private function renderParameter(ReflectionParameter $parameter): string
    {
        ...
        if ($parameter->isOptional() && !$parameter->isVariadic()) {
            ...
            // argument means for every litsea signature.
