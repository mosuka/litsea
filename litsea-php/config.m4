dnl config.m4 for the litsea PHP extension (Rust, built with ext-php-rs).
dnl
...
PHP_ARG_ENABLE([litsea],
  [whether to enable the litsea extension],
  [AS_HELP_STRING([--enable-litsea],
    [Enable the litsea word segmentation extension (needs a Rust toolchain)])],
  [yes])

if test "$PHP_LITSEA" != "no"; then
  dnl The Cargo workspace root is the parent of this directory. PIE extracts
  dnl the whole repository and runs phpize inside litsea-php/, so this holds
  dnl there too; a copy of litsea-php/ on its own cannot be built.
  AC_MSG_CHECKING([for the litsea Cargo workspace])
  if test -f "$abs_srcdir/../Cargo.toml"; then
    AC_MSG_RESULT([$abs_srcdir/..])
  else
    AC_MSG_RESULT([not found])
    AC_MSG_ERROR([$abs_srcdir/../Cargo.toml does not exist. litsea-php must be built from a full litsea repository checkout, not from a copy of this directory alone.])
  fi
  ...
  dnl where Makefile.frag writes litsea.so, gives it one directory to
  dnl create.
